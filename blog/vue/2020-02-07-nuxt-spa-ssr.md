...
나는 그동안 Nuxt를 제대로 알고있다고 생각했다. 흔히 말하는 SPA의 특징과 SSR의 특징을 이해하고 있다고 믿었고, 서비스의 특징과 필요에 따라 사용하면 된다고 알고있었다. 그러나 사이드프로젝트를 진행하면서 의문이 들어 나름 조사를 해보니 SPA와 SSR만 알면 Nuxt를 제대로 아는 것이 아니었더라. Static Web과 SPA, SSR의 차이를 알아보고 제대로 알고 쓰도록 하자.

---

# 📱 SPA?

**단일 페이지 애플리케이션(Single Page Application, SPA)** SPA란 단일 페이지로 구성된 웹앱을 말한다. 기존의 서버 사이드 렌더링 방식과 비교할 때, 배포가 간단하며 상대적으로 유려한 UX를 제공한다는 장점이 있다. SSR과 비교를 해보겠지만 장점과 단점을 알아보자.

## 장점

- SSR과는 다르게 페이지 이동시 깜빡임이 없다. (유려한 UI/UX 제공)
...
- 초기 로딩 속도가 느리다.
  - 앱이 커질수록 js의 용량도 커지다보니 초기 로딩 속도가 자연스레 느려진다.

---

# 🖥 SSR?

**서버 사이드 렌더링(Server Side Rednering)** SSR이란 SPA의 클라이언트 사이드 렌더링과는 반대로 유저가 화면을 요청하면 서버에서 미리 HTML을 그려 렌더링하는 방식이다. 장단점은 SPA(클라이언트 사이드 렌더링)과 대조된다.
...
간단하게 SPA(클라이언트 사이드 렌더링)와 SSR(서버 사이드 렌더링)의 차이를 알아보았다. 그렇다면 대체 `vue-cli`가 있음에도 `nuxt`는 왜 생겨났으며, 어떤 명령어를 통해서 어떻게 사용을 해야할 지 차근차근 알아보자.

## 1. vue-cli

일단 vue-cli로 프로젝트를 만들면 해당 프로젝트는 기본적으로 SPA프로젝트이다. SSR을 이용할 방법이 없는것은 아니다.

> [https://kr.vuejs.org/v2/guide/ssr.html](https://kr.vuejs.org/v2/guide/ssr.html)  
> [https://ssr.vuejs.org/](https://ssr.vuejs.org/)

위 두 링크를 참고하면 SSR 또는 Pre-Rendering이 가능하다.
...
위 처럼 해당 프로젝트를 만들고 VSCode로 폴더를 열어보면

![vue-cli-프로젝트-구조](https://user-images.githubusercontent.com/54297322/75431307-0444c580-5990-11ea-8dc5-161a442f5531.png)

이런 구조와 `package.json`파일의 명령어를 확인할 수 있다.

## 3. 프로젝트 빌드

package.json에는 스크립트가 작성되어 있는데, `serve`는 개발용 앱을 실행시킬 때, `lint`는 코드 스타일링과 관련한 스크립트이니 `build` 명령어를 실행해보자.

```bash
yarn build
```

성공적으로 빌드를 마치면 프로젝트 폴더에는 `dist`라는 폴더가 생겨있고 폴더안에는 빌드된 웹의 내용물이 들어있을 것이다.

![vue-cli-build](https://user-images.githubusercontent.com/54297322/75431870-d90ea600-5990-11ea-901a-0b512c935f9a.png)

저 결과물을 서버에 업로드하면 SPA는 배포가 되고 바로 접속이 가능하다. 이런 과정을 통해 SPA는 배포가 간단하다는 장점을 가질 수 있는 것이다.
...
> `npx serve` 명령어는 간단한 웹서버를 설정없이 바로 실행할 수 있게 도와준다. npx와 serve는 따로 찾아보거나 일단 넘어가도록 하자.

## 4. 결과 화면

![vue-cli-view](https://user-images.githubusercontent.com/54297322/75432458-c648a100-5991-11ea-89ec-87d54248a1bb.png)

`npx serve dist`를 통해 웹앱을 실행시키고 해당 주소로 들어가면 웹이 실행된걸 확인할 수 있다. 또한 크롬 콘솔(이미지는 네이버 웨일)을 통해 검사를 하며 body태그안에 화면을 구성하는 태그들이 있음을 알 수 있다.

이번엔 브라우저 화면을 마우스 우클릭하여 *페이지 소스보기*를 통해 살펴보자.

![vue-cli-view-source](https://user-images.githubusercontent.com/54297322/75432771-47079d00-5992-11ea-998f-0723778dedfd.png)

위의 화면은 페이지 소스보기를 통해 본 화면이다. 이미지의 우측하단 `<div id="app"></div>`부분을 살펴보면 크롬 콘솔로 본 결과와는 다르게 내용이 비어있는 것을 확인할 수 있다.

이처럼 SPA를 이용한 웹은 화면을 미리 그려놓지 않기 때문에 앞서 나열한 장단점을 갖게되는 것이다. 그렇다면 Nuxt는 어떨까??

---

# ✅ Nuxt를 이용하면..

Nuxt를 이용하여 프로젝트를 만들면 SSR을 사용할 수 있다고 하였다. 또 공식홈페이지를 살펴보면 Nuxt는 SPA또한 구현이 가능하다. 어떤 명령어를 사용하고 배포해야 이 일이 가능한지 알아보자.

> 명령어와 모드에 따른 차이를 작성하는 것이기 때문에 자세한 설명은 생략할 수 있습니다.
...
create-nuxt-app이라는 오픈소스를 통해 nuxt프로젝트를 만들어보았다. create-nuxt-app은 SSR을 사용할 서버를 선택할 수 있는데 기본제공되는 nuxt 프레임워크를 선택했다.

![nuxt-create-project](https://user-images.githubusercontent.com/54297322/75513507-678a3280-5a38-11ea-9750-cef441d23d00.png)

## 2. 모드

Nuxt에는 두 가지 프로젝트 모드를 제공합니다.

1. `spa`
2. `universal`

위 두 가지 모드를 통해 Nuxt는 SPA와 SSR 그리고 Static Web을 구현할 수 있습니다. 모드는 프로젝트의 `nuxt.config.js`에서 값을 바꾸어주어도 되고 스크립트를 실행할 때 `nuxt --spa`라는 추가 옵션을 붙여줌으로써도 실행이 가능합니다.

![nuxt-create-config](https://user-images.githubusercontent.com/54297322/75513690-eda67900-5a38-11ea-9223-a306f8e43e27.png)

`package.json`을 살펴보면 아래와 같이 기본적인 구성이 되어있고, 앞으로 spa와 universal모드를 비교해가며 각 스크립트를 실행해 보겠습니다.
...
dev는 로컬에서 개발목적으로 nuxt를 실행시키는 명령어입니다.

공식 홈페이지에서는 이렇게 설명하고 있습니다.

> 개발서버를 핫 리로딩 상태로 localhost:3000에 시작합니다.

#### 3.1.1. 모드 universal

먼저 universal 모드로 dev를 실행해보겠습니다.
...
dev를 실행시키기 전과의 차이는 프로젝트에 `.nuxt`라는 폴더가 생겼다는 것입니다. vue-cli의 dist와 같은 역할을 하는 것이죠. 일단 서버가 띄워졌으니 접속해보겠습니다. (http://localhost:3000)
...
위 두 이미지를 보면 아시겠지만 이전 vue-cli를 이용했을 때완 다르게 페이지소스보기에도 미리 작성된 코드들이 보이는 것을 알 수 있습니다.

그렇다면 SSR이 되었다는 것을 알 수 있겠죠. 미리 추가해둔 다른 페이지(http://localhost:3000/sub)도 같은지 확인해보겠습니다.
...
다른 route(url)를 가진 페이지도 마찬가지인 결과를 확인할 수 있습니다.

#### 3.1.2. 모드 spa

이번엔 spa 모드로 dev를 실행해보겠습니다.
...
spa 모드로 실행을 시켰더니, 보여지는 view는 같지만 페이지 소스보기에서 차이를 느끼실 수 있을겁니다.

#### 3.1.3. 정리

프로젝트의 모드를 `universal`과 `spa`중 뭘 선택하냐에 따라 개발서버를 실행했을 때에 결과가 달랐음을 알 수 있습니다.

### 4. build & start

build와 start는 묶어서 확인해보자. vue-cli의 build와 같은 역할을 합니다. 다만 빌드한 결과물을 start 명령어로 실행을 시킬 수 있습니다. production(배포)용 임을 인지하자.

build와 start를 공식홈페이지에서는 이렇게 설명하고 있습니다.
...
dev를 했을 때와는 다르게, .nuxt폴더 안에 dist폴더가 추가로 생겼습니다. 해당 파일들을 이용해 서버를 띄운다는 것 정도만 알고 넘어갑시다. 이어서 start를 해봅시다.
...
터미널을 보면 production 모드로 서버가 올라갔다는 것을 확인할 수 있습니다.
...
위 이미지를 살펴보면 dev 명령어를 universal 모드로 켰을 때와 비슷하지만, 다른 면이 보이실겁니다. 페이지 소스보기를 보면, html내용은 보이는 것이 확인됩니다. 즉 SSR이 되었다는 의미겠죠. 그러나 윗부분의 style코드가 dev모드에 비하면 상당히 압축되어있습니다. production(배포)용으로 빌드가 됐기 때문에 js 및 css 관련 코드들이 압축이 된것입니다.

바로 지금 이 상태가 **SSR을 이용해 배포를 한 상태**입니다.

**SSR은 초기 진입 페이지를 서버에서 불러와 페이지를 렌더링합니다. 즉 페이지를 그려주기 위한 서버가 필요합니다.**
이전 vue-cli를 보면 build의 결과물이 정적파일들만 있었고, 그 정적파일을 웹서버(nginx, github pages, netlify, amazon S3 등)에 올려서 접근만 하면 사용이 가능했습니다. SPA가 배포가 간단한 것은 정제된 정적파일을 올리기만 하면 된다는 점이고, SSR을 하기 위해선 이처럼 화면을 그려줄 서버가 추가로 필요하기 때문에 비용(과정, 난이도, 시간 등)이 더 들게됩니다.

그렇다면 SSR을 이용하면 SPA장점을 잃게 되는 것일까요? Nuxt는 초기진입만 서버를 이용하고 그 뒤론 SPA의 흐름을 따릅니다. 아래의 이미지를 봅시다.

![nuxt-universal-url](https://user-images.githubusercontent.com/54297322/75517570-56deba00-5a42-11ea-80f1-eba5265f6318.gif)

먼저 url로 접근을 시도하면 화면이 깜빡거리며 다시 그려지는 것을 콘솔을 통해 확인할 수 있습니다.

![nuxt-universal-link](https://user-images.githubusercontent.com/54297322/75517786-e4220e80-5a42-11ea-96f5-696e3b723be8.gif)

그러나 초기 진입 이후에 앱 내 링크를 통해 움직이면 위처럼 바뀐 화면부분만 렌더링이 되는 것을 확인할 수 있습니다.

결과적으로 모든 페이지가 SSR이 되는 장점도, 그 이후에는 SPA의 장점 또한 가질 수 있습니다.

적절한 방법을 선택하는 것이 좋겠습니다.
...
universal 모드와는 다르게, server폴더의 구성이 좀 다르네요. 이번에도 start를 해보겠습니다.
...
> mode를 spa로 바꾸지 않았다면 `--spa`옵션을 꼭 붙여주어야 합니다. 그렇지 않으면 실행이 되지 않습니다.
...
SPA모드로 실행이 됐습니다. dev 명렁어를 이용했을 때와 큰 차이는 없어보입니다. 다만 프로젝트가 production(배포)용으로 올라갔을 것입니다. 그 차이가 있겠네요.

하지만 SPA모드를 사용하는데에도 start를 이용해서 서버를 켜야한다면, vue-cli에 비해 뭐가 나은지 모르겠습니다. 그 점을 기억하고 마지막 명령어인 generate를 확인해보러 가보도록 합시다.

### 5. nuxt generate

gnerate 명령어는 앱을 빌드하고 정적파일을 생성합니다. 공식홈페이지의 설명을 확인해봅시다.
